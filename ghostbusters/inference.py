...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        d = [i[1] for i in items]
        v = [i[0] for i in items]
        r = random.random()
        i, total = 0, d[0]
        while r > total:
            i += 1
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPosition in self.allPositions:
            ...
            d[ghostPosition] = p * self.beliefs[ghostPosition]
        d.normalize()
        self.beliefs = d
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos in newPosDist.keys():
                d[newPos] += oldProb * newPosDist[newPos]
        self.beliefs = d
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPos in self.particles:
            ...
            for i in range(self.numGhosts):
                prob *= self.getObservationProb(observation[i], pacmanPos, ghostPos[i], self.getJailPosition(i))
            d[ghostPos] += prob
        if d.total() == 0:
            self.initializeUniformly(gameState)
        else:
